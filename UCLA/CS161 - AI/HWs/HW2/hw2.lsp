...
(defun BFS (FRINGE)
    ...
    (cond ((null FRINGE) ())
          ; If the current node's car is a list then recursively call BFS  to append its car to its cdr. 
          ((listp (car FRINGE)) (BFS (append (cdr FRINGE) (car FRINGE))))
          ; otherwise, recursively call BFS on the cdr of current node and append to car.
