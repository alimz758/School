...
bool StudentWorld:: doesOverlapsWithcitizenPenelope(double x, double y)
{
    for(vector<Actors*>::iterator iter = m_level.begin() ; iter !=m_level.end() ; iter++)
    {
        //if the current actor is penelope or citizen
        if((*iter)->isCitizen() || (*iter)==penelopePtr ||(*iter)->isZombie() )
        {
...
//check if it overlaps with pit, penelopee, citizen, zombies
void StudentWorld:: overlapWithPit(double here_x, double here_y)
{
    for (vector<Actors*>::iterator iter = m_level.begin() ; iter !=m_level.end() ; iter++ )
    {
        //if there is at least one citizen on the level, return false
        if( (*iter)->isCitizen() || (*iter)==penelopePtr || (*iter)->isZombie())
        {
            //When the person/zombie is destroyed, it must behave just as it were damaged by a flame
            (*iter)->flameDamagable(here_x, here_y);
        }
    }
}
